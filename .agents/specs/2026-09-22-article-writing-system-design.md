...
The whole public site is currently treated as one work because an invested reader can plausibly consume it in a single visit of less than 90 minutes. The assessment therefore covers all public routes and copy, not one article in isolation.
...
The system does not need user-session tracking or hypothetical route models now. When an invested reader could plausibly spend 90 minutes consuming new public content without rereading material, that threshold triggers an explicit design decision about plausible reader journeys and classification units. The site-wide rule must not silently relax or fragment before that decision is made.
...
18. The system flags the 90-minute threshold for a design decision without inventing journey rules.
...
When an invested reader could plausibly spend 90 minutes consuming new public content without repeated reads, create an explicit design decision that considers:

- whether the whole site remains the appropriate classification unit;
- which reader journeys are plausible rather than merely technically possible;
- how navigation, recommendation and recency affect cumulative exposure;
- whether language budgets belong to visits, content groups or another defensible unit;
- how any new model can be reviewed without invasive runtime tracking.

Until that decision is approved, the entire public site remains one work for the 12A house standard.
...
- The 90-minute growth threshold is recorded as a future design trigger, with no journey machinery introduced now.
