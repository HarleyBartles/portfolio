...
- Build a local command in the repo-owned `running-reader-panels` skill. It reads an explicitly named Markdown article and never ships in the Vite site or exposes an API key to browser code. The writing skill points to the panel when a developmental edit calls for it.
...
- The CLI consumes the frozen JSON through required `--profile-file`. Validate unique IDs, known archetype IDs, at most ten readers per archetype and at most 100 overall. The report keeps per-archetype denominators and individual trajectories; overall counts are a barometer, not the sole diagnostic.
