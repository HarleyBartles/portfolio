...
repo's `.agents/runbooks/<stage>.md` as its own first step. That runbook is
the local stage composition root and resolves every applicable conditional
composition under `.agents/playbooks/`. For the
