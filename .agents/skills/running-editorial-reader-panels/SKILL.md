...
The default ten profiles are concise reader intents. `assets/reader-intents-rich.json` preserves the ten richer archetypes used in the pilot. [The curated pool](assets/reader-archetypes.json) contains 34 distinct arrival intents; [the catalogue guide](references/reader-archetype-catalogue.md) helps select a relevant panel. Use `reader_panel.py --profile-file` and `--profiles` to try selected archetypes directly. To build a 100-reader cohort, give `reader_panel_cohort.py` that pool with `--archetypes <ten-comma-separated-ids>`, check it, then apply with `--output <absolute-scratch-path>`. Give its output to `reader_panel.py --profile-file <path>`. The current generator adds ten different reading lenses to each selected archetype. Keep experimental cohorts in scratch until a useful version earns promotion to the repository.
