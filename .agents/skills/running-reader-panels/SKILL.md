...
For a paid run, set `OPENROUTER_API_KEY` in the process environment and pass `--apply --max-calls <count> --max-usd <amount>`. The SDK runs independent reader-condition journeys concurrently, while each journey stays sequential. `--concurrency` bounds active requests. The harness reserves global call and estimated-spend capacity before sending work, counts retries per request, and reconciles reported usage. If a failed request has no returned usage, its attempts and provisional cost estimate are reported as unpriced and new requests stop. Before resuming that checkpoint, reconcile those attempts in provider billing and pass the actual total with `--reconciled-unpriced-usd <amount>`; the harness will not resume them on an estimate alone. Costs are estimates until reported or externally reconciled. Progress is printed while running. Completed journeys are checkpointed atomically in canonical off-repo scratch; `--resume <partial-report>` continues only when source, manifest, frozen cohort, model and rendered-prompt fingerprints still match. Incomplete journeys remain in the checkpoint and are excluded from journey and observation results. Optional-read summaries include combined counts and denominators, plus breakdowns by condition, core outcome and archetype. Only exposed text goes to OpenRouter's Jev Decisions API. The older `--article` mode remains for flat-text runs and does not model optional content.
