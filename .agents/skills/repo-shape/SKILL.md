...
Each repo supplies a thin overlay at `.agents/doctrine/repo-runbook-policy.md`
that records local mappings and shape exceptions. `repo-composition` owns the
meaning of runbooks, playbooks, and their graph.
...
`repo-shape` is a check-and-align tool, not a first-turn workflow router. Do
not invoke it before `using-superpowers-plus`.
