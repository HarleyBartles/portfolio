...
| Access Checks proves responsibility but not yet one consequential outcome | `professionalProfile.ts` records the formal title, sole-engineer scope and public system shape; About does not yet contain a stakes, change, outcome and influence chain | important discovery, source-gated | Ask Harley for one employer-safe example before API drafting, record its disclosure class, use only approved facts in the essay, and hand the account to Phase 7A for canonical About and CV treatment |
...
**Intended response:** A weary hiring-manager skeptic should recognise a software engineer applying established product, delivery, testing and operational judgement to agentic systems. The index and Vibe lead must offer an early route into API ownership or evidence-led testing, so the publication cannot be mistaken for commentary about agents alone. The publication should feel useful in its own right. Hiring value remains subtext.
...
**Factual, privacy and custody boundaries:** Pin every cross-repository claim. Access Checks copy stays inside the approved public facts and must not expose internal topology, customer identity, candidate data or private metrics. It may name the product, .NET 8, Azure Functions, customer portal, Access Screening as a downstream consumer, and Harley's end-to-end responsibility. Any claim about dependants, stakes, a before state, outcome or organisational influence requires Harley's explicit approval and a recorded exact, approximate, qualitative or confidential classification. Source relationships must say derivation, traditional composition, Harley's extension, later convergence, supporting evidence or further reading accurately.
...
**Observable acceptance signals:** Exactly five essays exist publicly. One and only one index lead is visually obvious. The index exposes a core-engineering doorway without a long scan, and Vibe links directly to API or Testing through one authored continuation. Each first viewport states one useful proposition and contains a readable principal figure whose text alternative carries the same argument. A reader can follow the approved continuation graph without seeing hidden category labels. Every essay has a concrete opening, earns its sections, uses evidence honestly and ends quietly. The API and testing essays replace positioning with concrete product responsibility, risk, test boundaries and operational evidence without duplicating the Wild Bunch architecture case study. At 320 pixels and 200% zoom, prose, figures and continuation choices remain legible and free of horizontal overflow.
...
- [ ] **Step 1: Lock the API argument and evidence boundary before drafting.** Use Access Checks only as the safe concrete anchor. Ask Harley for one strong employer-safe example covering who or what relies on the product, the consequence when it is unavailable or wrong, the before condition, Harley's exact decision, the observable result, the effect on adjacent people or teams, and whether each fact is exact, approximate, qualitative or confidential. Record the approved source boundary for Phase 7A as well as this essay. Structure the piece through ambiguity, contract, implementation, delivery, operation, support, recovery and learning. Keep the API visibly in the middle. Name only approved public facts and never imply original authorship of the whole product architecture. If no safe example survives review, omit outcome claims rather than inventing them and leave Phase 7A blocked at its source gate.
...
- [ ] **Step 7: Run a whole-publication editorial pass.** Sol reads the index and five essays in the curated traversal, checking that every sentence pays dues, concepts do not repeat without purpose, terminology is stable, source links earn their place and the publication sounds like one human author. Run the weary-skeptic route from the index and Vibe lead, recording where concrete conventional-engineering evidence first appears and confirming that API or Testing is reachable without traversing another agentic-only essay. Run a repo-wide frequency scan for familiar AI constructions rather than treating each article in isolation.
