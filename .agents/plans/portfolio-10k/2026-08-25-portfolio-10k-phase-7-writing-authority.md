...
- [ ] **Step 12: Commit and close the roadmap record.** Commit `feat: finish the writing authority publication`. After merge proof, record exact commit, PR, rating and hosted run in the roadmap, promote any durable decisions, remove this plan from the tracked tree under `.agents/doctrine/completed-artifacts.md`, then use `/finishing-a-development-branch` for cleanup.
