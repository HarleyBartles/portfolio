...
- Hard budgets may not increase: JavaScript 358400 bytes, CSS 40960 bytes, CV PDF 524288 bytes, CV exactly 2 pages.
...
- all files named in Tasks 2-6

**Write scope:** this plan file only.

- [ ] Run `git fetch origin main --prune`, then `git rev-parse HEAD` and `git rev-parse origin/main`. Both must equal `5cdf3dcfa3faa1d98d119154203f40727bbfcb63`; otherwise stop for Sol to refresh the worktree and plan.
...
- [ ] Run the falsifier procedure for each row. Replace every “starting action” with the chosen action and add one observable pass/fail sentence. Do not write “TBD”, “looks better”, “cleaner”, or another unsupported taste adjective.
- [ ] Add one row for any visible treatment discovered but not listed above.
- [ ] Mark these checkboxes complete; do not commit yet.
...
**Expected result:** every in-scope visual treatment has one action and one content-specific reason before component edits begin.
...
- [ ] Apply the recorded header decision. Remove decorative rules or modifiers only if the record classifies them `remove-furniture`.
...
## Task 3: Localise Specialist Article Bodies and Inline Evidence
...
- Testing Evidence and Context Complexity wrappers must not gain furniture merely to look componentised.
- Article copy and shared `ContentProse` / `EditorialAside` behaviour are protected.

- [ ] Add or strengthen tests for article semantics and lazy fallback behaviour before changing implementation.
- [ ] Replace class-hook composition with locally named styled components.
- [ ] Apply each recorded keep/refine/remove decision; preserve screen-reader reading order.
- [ ] Remove the Sass import and delete the file only after `rg` proves no selectors remain.
...
**Expected result:** each body owns its composition; the signal map remains evidence rather than decorative furniture.

## Task 4: Localise the Three Sequential Header Figures
...
- [ ] Apply recorded creative decisions one figure at a time and compare against its baseline at all four widths.
...
## Task 5: Localise the Artifact and Organisation Figures
...
- [ ] Apply recorded creative decisions, checking that removal does not erase grouping or hierarchy.
...
## Task 6: Localise the Rian Hughes Typographic Study
...
- [ ] Apply the recorded creative decisions and inspect every wordmark at 1440, 768, 390, and 320 widths.
...
## Task 7: Integrate, Prove, and Publish the Implementation
...
- Update intentional snapshots under `src/client/e2e/*-snapshots/` only after visual review
...
- [ ] Run `npm run test:e2e:visual` twice without snapshot updates. If an intentional decision changes a protected image, run `npm run test:e2e:visual -- --update-snapshots` once, inspect the image diff, then run `npm run test:e2e:visual` twice without updates.
- [ ] Run `npm run build`; expected result: successful Vite production build.
- [ ] Compare built JavaScript and CSS sizes with the hard budgets; expected result: neither budget increases.
...
- [ ] Can Luna remove valueless furniture without asking permission for each reversible local decision?
...
After plan approval, invoke `/executing-plans` with `gpt-5.6-luna` at `high` reasoning. Execute Tasks 1-7 sequentially. Sol reviews each return packet and diff before dispatching the next packet. Stop for Harley only at the explicit Global Constraints gates or when the retry threshold is reached.
