...
- Preserve the durable non-home design contract: cool-mineral substrate, Source Sans 3 shared site voice, Source Serif 4 only for authored longform, Source Code Pro only for genuine technical material, quiet single-flow shell, artifact-first evidence, and relationship-led cadence.
- Do not redesign or migrate the homepage movements in Slices A–B. The accepted homepage uses the mineral substrate and bounded project-earned colour; warm paper and copper are historical inputs, not protected defaults. Slice G remains a later, separate plan.
...
- [x] Replace the blunt barrel rule with shared/eager and narrow cohesive barrels only. Vite proof: after the final prose and header-style ownership moves, the common components chunk is 213.40 kB versus the 220.02 kB repair baseline; `ProjectVisual` remains a separate 6.10 kB JS / 10.34 kB CSS shared-lazy chunk; `VibeCodingFigure` remains a separate 1.47 kB JS / 3.04 kB CSS chunk; homepage, case-study and article presentations remain separately emitted. Entry JS is 212,351/460,800 bytes, CSS 22,771/40,960 bytes, and PDF 220,107/524,288 bytes.
...
- Slice G — homepage last: create a separate plan/PR for meaningful movement-internal seams and parent-owned static descriptors. Preserve the accepted deterministic edition, route-owned teaser model, no-autoplay behaviour, mineral shared ground, bounded project-earned colour, and existing homepage visual baselines; do not replace those relationships with a generic house treatment.
