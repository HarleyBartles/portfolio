...
- Significant source subtrees expose one intentional import root (`components`, `data`, `styles`, `types`, and `utils`). Consumers outside a subtree import from its `index.ts`; implementation files inside a subtree may use local modules. Feature directories remain route-local so a barrel cannot defeat lazy loading.
...
- Harley has explicitly authorized a provisional A–B JavaScript ceiling of 460,800 bytes (the checker expresses this as 450 * 1024) so the common styled runtime does not block the re-architecture. CSS remains capped at 40,960 bytes and PDF remains governed by the existing generator limit. Tighten the JavaScript ceiling in a later performance baton after A–B settles; do not treat this provisional ceiling as a design target.
...
- [x] Step 1: Build and measure the common boundary. The provisional user-authorized ceiling holds; the final Slice B/import-root build reports entry JS 212,261/460,800 bytes, CSS 22,905/40,960 bytes, PDF 220,107/524,288 bytes.
...
- [x] Step 2: Verify build/runtime proof. Run npm run build and record entry JS/CSS/PDF bytes from check-build-budget.mjs beside the Slice A measurement. Confirm the common styled runtime remains below the provisional 460800 JS ceiling and 40960 CSS bytes, route chunks stay lazy, direct article/index routes have no visible unstyled first state, and blocked Source-font requests still produce readable no-overflow output. Evidence: entry JS 212,261/460,800, CSS 22,905/40,960, PDF 220,107/524,288; build and budget pass.
- [x] Step 3: Review visual and semantic diffs. Inspect before/after renders at the four required widths and the protected route contracts. If output differs, treat it as a migration defect and repair component ownership, props or CSS-variable use. If the difference is instead an unresolved hierarchy/treatment collision, stop and surface the competing visual contracts to Harley; do not choose a collapse or update a baseline. Evidence: the only observed differences were the explicitly accepted stale-font baselines; no hierarchy ambiguity was collapsed.
- [x] Step 4: Commit and publish the implementation checkpoint normally. Stage the intended Slice B source/tests and generated mesh, commit through the tracked hook, push the implementation branch/PR, and keep it draft until Harley's visual acceptance. Hosted checks are evidence only after the published head is verified. Evidence: commit `bf69a1bac597f34518585403e170c76cc42774da` is published at draft PR #51 with matching head.
- [x] Step 5: Mark the task complete in this plan. The tracked hook passed repository, unit, build/budget and 140-browser gates; mesh is current; the published draft PR head is verified.
...
- Scope check: No homepage movement, project-native evidence rewrite, visual redesign, token-value change, or C–G migration is included in this plan PR. The user explicitly authorized a provisional A–B JavaScript ceiling increase to 460,800 bytes; CSS/PDF limits remain unchanged and later tightening is a durable follow-up.
- Adversarial review: The pass searched all route/page/component TSX for caller-owned layout recipes, derived modifier classes and untyped React declarations. Shared seams now use typed `ContentArticle`, `ArticleBody`, `ContentHeader`, `IndexHeader`, `StatePanel`, `WritingHeaderVisual`, status transient props and explicit index-entry components. Remaining feature-native class variants (About/CV, case-study evidence, specialist writing figures, Patch showcase and homepage movements) are recorded as C–G batons; no ambiguous same-hierarchy treatment was collapsed without an accepted visual contract.
- Import-root review: Components, data, styles, types and utils now have deliberate barrel boundaries. Feature barrels remain intentionally absent to preserve lazy route chunks; direct implementation imports inside each subtree remain allowed to avoid cycles.
