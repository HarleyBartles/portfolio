...
- [x] Step 2: Verify build/runtime proof. Run npm run build and record entry JS/CSS/PDF bytes from check-build-budget.mjs beside the Slice A measurement. Confirm the common styled runtime remains below the provisional 460800 JS ceiling and 40960 CSS bytes, route chunks stay lazy, direct article/index routes have no visible unstyled first state, and blocked Source-font requests still produce readable no-overflow output. Evidence: entry JS 212,346/460,800, CSS 22,595/40,960, PDF 220,107/524,288; build and budget pass. The emitted graph keeps `components` at 210.32 kB, `ProjectVisual` at 6.10 kB JS/10.13 kB CSS, `VibeCodingFigure` at 1.47 kB JS/3.04 kB CSS, and homepage/case-study/article routes separately emitted.
...
- [x] Replace the blunt barrel rule with shared/eager and narrow cohesive barrels only. Vite proof: the final build records the common components chunk and keeps project visuals, Vibe art, homepage code, case-study presentations and article routes separately emitted; route-local feature implementations are absent from the root barrel. Entry JS/CSS/PDF measurements are recorded with the final validation evidence below.
