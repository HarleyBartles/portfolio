...
## The Usual Specialists Index accepted assets
...
- Source/derivative boundary: the accepted PNG masters are durable reproduction inputs, not public site media. `src/client/scripts/process-usual-specialists-assets.mjs` verifies each committed master against `accepted-assets.json` before producing one public WebP derivative under `src/client/public/media/patch/the-usual-specialists/`. The generated `usual-specialists-derivatives.json` receipt records source hash, public path, output dimensions, encoding, byte count and output SHA-256.
...
- Generated-image provenance boundary: these are the already accepted PORT-16 visual outputs being promoted into durable custody. The portfolio processor does not generate replacement art, infer lost prompts, or lend scratch-only/rejected candidates accepted status. Known Index traversal provenance is Linear document `a6163be5-3bdd-425a-b144-b00e3a88423d`; the accepted Patch return continuation is recorded in `72833c08-cbbe-4674-889a-9024c307c415`.
...
- Silk diegetic-wall custody boundary: the 12 September PORT-18 direction supersedes the first rectangular Silk wireframe before any generated Silk scene was accepted into production custody. `src/client/assets/patch/the-usual-specialists/silk/` is reserved for future **accepted** Silk source masters and their provenance only; it intentionally contains no `accepted-assets.json` yet. The first Commission 05 generation from the superseded corridor-panel brief was rejected during visual QA and is **not** an accepted source asset, production derivative or custody input. When the first Silk master is accepted, record its exact source identity, generation/provenance document, intrinsic dimensions and acceptance state in that chapter-specific package before extending the deterministic media processor. Do not append speculative/rejected Silk imagery to the current Index-only manifest.
