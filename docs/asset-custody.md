...
Portfolio is temporary canonical source/provenance custody for the active *The Usual Specialists* V2 image work. The live V2 source package is currently Index-only under `src/client/assets/patch/the-usual-specialists/index/`; any future page-review candidate must stay in scoped `candidates/` custody until explicit acceptance or abandonment. Detailed human provenance lives under `src/client/assets/patch/the-usual-specialists/provenance/`, the Index `generation-receipt.json` records genuinely known generation metadata, and `src/client/public/media/patch/the-usual-specialists/usual-specialists-derivatives.json` is the authority for deterministic page-use derivative identity. Linear is migration history only; `provenance/linear-source-register.json` retains only records still represented by live V2 custody.

- Source/derivative boundary: accepted Index PNG masters and explicitly selected Index page-review candidate PNG masters are durable reproduction inputs, not public site media. `src/client/scripts/process-usual-specialists-assets.mjs` verifies the Index manifests, normalized provenance graph, generation receipt and public derivatives. `npm --prefix src/client run media:usual-specialists:apply` is the only derivative writer; `npm --prefix src/client run media:usual-specialists:check` writes nothing and fails on source, provenance, derivative or receipt drift.
