...
Portfolio is temporary canonical source/provenance custody for the accepted *The Usual Specialists* image work. Long-term promotion to the Adventures of Patch repository is a separate future custody action. Accepted source masters live under `src/client/assets/patch/the-usual-specialists/index/`, `rope/`, and `silk/`; source manifests answer which bytes are accepted in each package. Detailed human provenance now lives under `src/client/assets/patch/the-usual-specialists/provenance/`, package `generation-receipt.json` files record genuinely known generation metadata, and `src/client/public/media/patch/the-usual-specialists/usual-specialists-derivatives.json` is the authority for deterministic page-use derivative identity. Linear is migration history only; `provenance/linear-source-register.json` records deletion readiness.

- Source/derivative boundary: accepted PNG masters are durable reproduction inputs, not public site media. `src/client/scripts/process-usual-specialists-assets.mjs` verifies each committed master against the owning `accepted-assets.json`, verifies the normalized provenance graph and generation receipts, and verifies public derivatives. `npm --prefix src/client run media:usual-specialists:apply` is the only writer; `npm --prefix src/client run media:usual-specialists:check` writes nothing and fails on source, provenance, derivative, or receipt drift.
