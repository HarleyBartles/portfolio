...
- Transformation: `src/client/scripts/process-learning-lab-assets.mjs` verifies master hashes and dimensions, applies the accepted focal crop or inside fit, exports AVIF/WebP and strips metadata. `npm run media:learning-lab:check` fails when a master, derivative or receipt drifts.
