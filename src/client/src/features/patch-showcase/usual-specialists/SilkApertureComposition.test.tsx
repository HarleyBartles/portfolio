...
describe('SilkApertureComposition', () => {
  ...
  test('presents Commission 07 as candidate frames over the same parallax-ready service-corridor image', () => {
    ...
    const world = root?.querySelector<HTMLImageElement>('[data-silk-commission-07-review-world]')
    const frame = root?.querySelector<HTMLImageElement>('[data-silk-commission-07-review-frame]')
    const portraitFrame = root?.querySelector<HTMLImageElement>('[data-silk-commission-07-review-portrait-frame]')
    ...
    expect(viewport).toBeInTheDocument()
    expect(world).toHaveAttribute('src', expect.stringContaining('silk-commission-07-service-corridor-review.webp'))
    expect(world).toHaveAttribute('width', '1672')
    expect(world).toHaveAttribute('height', '766')
    expect(world).toHaveAttribute('alt', '')
    ...
    expect(portraitFrame).toHaveAttribute('src', expect.stringContaining('silk-commission-07-frame-review-portrait.webp'))
    expect(portraitFrame).toHaveAttribute('width', '941')
    expect(portraitFrame).toHaveAttribute('height', '1671')
