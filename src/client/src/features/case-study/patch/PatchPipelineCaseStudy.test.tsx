...
describe('PatchPipelineCaseStudy', () => {
  ...
  test('keeps published, in-flight, and story-lab boundaries public-safe', () => {
    ...
    const identityEvidence = within(inFlight).getByRole('figure', { name: /Identity Emporium compares three approaches to preparation/i })
    ...
    expect(identityEvidence).toHaveTextContent(/Bot[\s\S]*preparation mistaken for a script/i)
    expect(identityEvidence).toHaveTextContent(/Bit[\s\S]*straight to work, underprepared/i)
    expect(identityEvidence).toHaveTextContent(/Patch[\s\S]*preparation shaped by the task/i)
    expect(identityEvidence).toHaveTextContent(/misreads the work/i)
    expect(identityEvidence).toHaveTextContent(/starts too soon/i)
    expect(identityEvidence).toHaveTextContent(/ready to work/i)
