...
test('homepage keeps its authored masthead and feature composition', async ({ page }) => {
  ...
  await expect(page.locator('.hero')).toHaveScreenshot('homepage-masthead.png')
  await expect(page.locator('.feature-section')).toHaveScreenshot('homepage-feature-deck.png')
})
...
test('writing index keeps a distinctive editorial lead', async ({ page }) => {
  ...
  await expect(page.locator('.editorial-card--featured')).toHaveScreenshot('writing-editorial-lead.png')
})
...
test('about page keeps the professional proof panel', async ({ page }) => {
  ...
  await expect(page.locator('.about-proof')).toHaveScreenshot('about-professional-proof.png')
})
...
test('article header keeps its hierarchy on mobile', async ({ page }) => {
  ...
  await expect(page.locator('.content-page-header')).toHaveScreenshot('article-mobile-header.png')
})
