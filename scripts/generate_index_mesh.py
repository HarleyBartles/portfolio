...
def is_derived_skills_root(path: Path) -> bool:
    try:
        return path.relative_to(ROOT).as_posix() == ".agents/skills"
    except ValueError:
        return False


def load_derived_skill_names(skills_root: Path) -> set[str]:
    provenance_path = skills_root / ".provenance.json"
    if not provenance_path.is_file():
        raise ValueError(f"Derived skills provenance is missing: {provenance_path}")

    provenance = json.loads(provenance_path.read_text(encoding="utf-8"))
    if provenance.get("schema_version") != 1:
        raise ValueError(f"Unsupported derived skills provenance schema: {provenance.get('schema_version')!r}")
    copied_skills = provenance.get("copied_skills")
    if not isinstance(copied_skills, list):
        raise ValueError("Derived skills provenance copied_skills must be a list")

    names: set[str] = set()
    normalized_names: set[str] = set()
    for raw_name in copied_skills:
        if (
            ...
            raise ValueError(f"Invalid derived skill name in provenance: {raw_name!r}")
        normalized_name = raw_name.casefold()
        if normalized_name in normalized_names:
            raise ValueError(f"Duplicate derived skill name in provenance: {raw_name}")
        skill_root = skills_root / raw_name
        if not skill_root.is_dir():
            raise ValueError(f"Derived skill listed in provenance is missing: {skill_root}")
...
def render_index(path: Path) -> str:
    ...
    dirs: list[Path] = []
    files: list[Path] = []
    derived_skill_names = load_derived_skill_names(path) if is_derived_skills_root(path) else None
    ...
    lines.append(
        ...
        r"`.\scripts\generate_index_mesh.ps1` on Windows or "
        "`bash ./scripts/generate_index_mesh.sh` on Linux, "
    ...
    if not repos and not dirs and not files:
